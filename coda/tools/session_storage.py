...
class ToolInvocation(BaseModel):
    ...
    message_id: Optional[str] = Field(None, description="ID of the message that triggered this invocation")
    ...
    # Execution timing
    created_at: datetime = Field(default_factory=datetime.utcnow, description="When invocation was created")
    ...
    # User interaction
    user_approved: Optional[bool] = Field(None, description="Whether user approved dangerous operation")
    approval_prompt: Optional[str] = Field(None, description="Prompt shown to user for approval")
...
class SessionToolSummary(BaseModel):
    ...
    # Timing information
    first_invocation: Optional[datetime] = Field(None, description="Time of first tool invocation")
    last_invocation: Optional[datetime] = Field(None, description="Time of last tool invocation")
    total_execution_time: float = Field(0.0, description="Total execution time in seconds")
    
    # Most used tools
    tool_usage_count: Dict[str, int] = Field(default_factory=dict, description="Count of each tool used")
    
    @classmethod
    def from_invocations(cls, session_id: str, invocations: List[ToolInvocation]) -> "SessionToolSummary":
        """Create summary from list of tool invocations."""
        if not invocations:
            return cls(session_id=session_id)
        
        successful = [inv for inv in invocations if inv.status == ToolInvocationStatus.COMPLETED and inv.result and inv.result.success]
        failed = [inv for inv in invocations if inv.status == ToolInvocationStatus.FAILED or (inv.result and not inv.result.success)]
...
class ToolInvocationFilter(BaseModel):
    ...
    tool_servers: Optional[List[str]] = Field(None, description="Filter by tool servers")
    
    # Status filters
    statuses: Optional[List[ToolInvocationStatus]] = Field(None, description="Filter by invocation status")
    success_only: Optional[bool] = Field(None, description="Only successful invocations")
    failed_only: Optional[bool] = Field(None, description="Only failed invocations")
    dangerous_only: Optional[bool] = Field(None, description="Only dangerous invocations")
    ...
    duration_min: Optional[float] = Field(None, description="Minimum execution duration in seconds")
    duration_max: Optional[float] = Field(None, description="Maximum execution duration in seconds")
    
    # Content filters
    search_text: Optional[str] = Field(None, description="Search in arguments and results")
...
def update_invocation_result(
    ...
    invocation.status = ToolInvocationStatus.COMPLETED if result.success else ToolInvocationStatus.FAILED
