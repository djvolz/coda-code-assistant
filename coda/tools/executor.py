...
from coda.providers.base import Tool, ToolCall, ToolResult
from coda.tools.base import ToolRegistry, tool_registry
    ...
    async def execute_tool_call(self, tool_call: ToolCall) -> ToolResult:
        ...
        try:
            ...
            if hasattr(tool, 'dangerous') and tool.dangerous:
                ...
                return ToolResult(
                    tool_call_id=tool_call.id,
                    content=f"Error: Tool '{tool_call.name}' requires manual approval",
                    is_error=True
                )
        ...
        for tool in tools:
            ...
            if tool.parameters:
                ...
                for param_name, param_info in tool.parameters.items():
                    ...
                    lines.append(f"    - {param_name}: {param_info['type']}{required} - {param_info.get('description', '')}")
