...
from .base import tool_registry, ToolSchema, ToolResult
...
class MCPServer:
    ...
    async def handle_mcp_websocket(self, request: web.Request) -> web.WebSocketResponse:
        ...
        try:
            ...
            logger.info(f"MCP WebSocket client disconnected. Remaining clients: {len(self.clients)}")
        ...
        try:
            ...
            mcp_result = {
                "content": [
                    {
                        "type": "text",
                        "text": str(result.result) if result.result else (result.error or "No output")
                    }
                ],
