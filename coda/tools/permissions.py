...
class PermissionRule(BaseModel):
    ...
    tool_names: Optional[List[str]] = Field(None, description="Specific tools this rule applies to")
    tool_categories: Optional[List[ToolCategory]] = Field(None, description="Tool categories this rule applies to")
    tool_patterns: Optional[List[str]] = Field(None, description="Tool name patterns (regex)")
    ...
    max_daily_usage: Optional[int] = Field(None, description="Maximum uses per day")
    allowed_hours: Optional[List[int]] = Field(None, description="Hours when tool can be used (0-23)")
    allowed_days: Optional[List[int]] = Field(None, description="Days when tool can be used (0-6, Mon-Sun)")
    
    # Parameter restrictions
    parameter_restrictions: Dict[str, Any] = Field(default_factory=dict, description="Restrictions on tool parameters")
    
    # Metadata
    created_at: datetime = Field(default_factory=datetime.utcnow)
    expires_at: Optional[datetime] = Field(None, description="When this rule expires")
...
class UserPermissions(BaseModel):
    ...
    base_permission: PermissionLevel = Field(PermissionLevel.STANDARD, description="Base permission level")
    
    # Applied rules
    rules: List[PermissionRule] = Field(default_factory=list, description="Permission rules for this user")
    
    # Usage tracking
    daily_usage: Dict[str, int] = Field(default_factory=dict, description="Tool usage count per day")
    last_usage_date: Optional[str] = Field(None, description="Last date usage was tracked (YYYY-MM-DD)")
    
    # Approval tracking
    pending_approvals: List[str] = Field(default_factory=list, description="Tool invocations awaiting approval")
    approved_operations: Set[str] = Field(default_factory=set, description="Recently approved dangerous operations")
    ...
    def get_effective_permission(self, tool_name: str, tool_schema: ToolSchema) -> PermissionLevel:
        ...
        applicable_rules = [
            rule for rule in sorted(self.rules, key=lambda r: r.priority, reverse=True)
    ...
    def can_execute_tool(self, tool_name: str, tool_schema: ToolSchema, arguments: Dict[str, Any]) -> tuple[bool, str]:
                ...
                for param_name, restriction in rule.parameter_restrictions.items():
                    if param_name in arguments:
                        ...
                        if not self._check_parameter_restriction(param_name, value, restriction):
                            return False, f"Parameter {param_name} violates restriction (rule: {rule.name})"
    ...
    def approve_operation(self, tool_name: str, arguments: Dict[str, Any], expires_in_minutes: int = 60) -> str:
...
class PermissionManager:
    ...
    def get_user_permissions(self, user_id: str) -> UserPermissions:
        ...
        if user_id not in self.users:
            self.users[user_id] = UserPermissions(
                user_id=user_id,
    ...
    def check_tool_permission(
        ...
        requires_approval = user_perms.requires_approval(tool_name, tool_schema) if can_execute else False
