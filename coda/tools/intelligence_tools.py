...
from coda.tools.base import BaseTool, ToolSchema, ToolParameter, ToolParameterType, ToolResult, tool_registry
from coda.intelligence import TreeSitterAnalyzer, RepoMap, DependencyGraph
        ...
        try:
            ...
            if include_cycles and stats.circular_dependencies:
                result["circular_dependencies"] = [
                    [Path(f).name for f in cycle]
                    for cycle in stats.circular_dependencies[:10]
                ]
...
class MapRepositoryTool(BaseTool):
    ...
    async def execute(self, arguments: dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(
                success=False,
                error=f"Error mapping repository: {str(e)}",
                tool="map_repository"
            )
