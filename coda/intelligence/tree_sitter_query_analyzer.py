...
class TreeSitterQueryAnalyzer:
    ...
    def _analyze_with_tree_sitter(self, file_path: Path, content: str, language: str) -> FileAnalysis:
        ...
        try:
            ...
            for node, tag in all_nodes:
                ...
                # Get node text
                node_text = node.text.decode('utf-8') if hasattr(node, 'text') else content[node.start_byte:node.end_byte]
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            for pattern, kind in patterns['definitions']:
                ...
                if match:
                    ...
                    definitions.append(CodeElement(
                        name=name,
                        kind=kind,
                        line=line_num,
                        column=match.start(),
    ...
    def _get_regex_patterns(self, language: str) -> Dict[str, List[Tuple[re.Pattern, DefinitionKind]]]:
        ...
        patterns = {
            ...
            'javascript': {
                ...
                'imports': [
                    ...
                    (re.compile(r'^\s*const\s+.*\s*=\s*require\([\'"]([^\'"]+)'), DefinitionKind.IMPORT),
                ]
            },
            'typescript': {
                'definitions': [
    ...
    def find_definition(self, name: str, analyses: Union[str, Dict[str, FileAnalysis]], 
                       kind: Optional[DefinitionKind] = None) -> List[CodeElement]:
        """
    ...
    def get_structured_analysis(self, file_path: Union[str, Path]) -> Dict:
        ...
        classes = [element_to_dict(d) for d in analysis.definitions if d.kind == DefinitionKind.CLASS]
        functions = [element_to_dict(d) for d in analysis.definitions if d.kind == DefinitionKind.FUNCTION]
        methods = [element_to_dict(d) for d in analysis.definitions if d.kind == DefinitionKind.METHOD]
        variables = [element_to_dict(d) for d in analysis.definitions if d.kind == DefinitionKind.VARIABLE]
        constants = [element_to_dict(d) for d in analysis.definitions if d.kind == DefinitionKind.CONSTANT]
        ...
        return {
            ...
            'file_info': {
                ...
                'language': analysis.language,
                'lines': len(Path(analysis.file_path).read_text().splitlines()) if Path(analysis.file_path).exists() else 0
            },
            'classes': classes,
            'functions': functions,
            'methods': methods,
            ...
            'definitions': {
                ...
                'variables': variables,
                'constants': constants,
                'other': [element_to_dict(d) for d in analysis.definitions if d.kind not in [
                    DefinitionKind.CLASS, DefinitionKind.FUNCTION, DefinitionKind.METHOD,
