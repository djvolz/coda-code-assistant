...
def create_command_selector(
    command_name: str,
    registry_commands: List[CommandDefinition],
    console: Console = None
) -> Optional[GenericCommandSelector]:
    """
    ...
    if (command_def.subcommands or 
        command_name in ["mode", "provider", "theme", "export", "session"]):
