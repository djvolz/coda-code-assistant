...
class OptionCompleter(Completer):
    ...
    def __init__(self, options: List[Tuple[str, str, Optional[dict[str, Any]]]]):
        """
...
class CompletionSelector:
    ...
    def create_prompt_session(self) -> PromptSession:
        ...
        style_dict = {
            ...
            'completion-menu.completion.current': prompt_styles.get('completion.current', f'bg:{self.console_theme.success} fg:black'),
            'completion-menu.meta.completion': prompt_styles.get('completion.meta', 'italic'),
            'completion-menu.meta.completion.current': prompt_styles.get('completion.meta', 'italic') + ' ' + prompt_styles.get('completion.current', ''),
    ...
    async def select_interactive(self, auto_complete: bool = True) -> Optional[str]:
        ...
        try:
            ...
            if result in valid_values:
                ...
                self.console.print(f"[{self.console_theme.error}]Invalid selection: {result}[/{self.console_theme.error}]")
                return None
