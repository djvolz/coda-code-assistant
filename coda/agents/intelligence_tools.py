...
def find_definition(name: str, kind: Optional[str] = None) -> str:
    """
    ...
    """
    try:
        from coda.intelligence.tree_sitter_query_analyzer import DefinitionKind
        ...
        for defn in definitions:
            ...
            relative_path = file_path.relative_to(directory) if file_path.is_absolute() else file_path
...
def analyze_code(file_path: Optional[str] = None) -> str:
    """
...
def code_stats(directory: Optional[str] = None) -> str:
    """
    ...
    try:
        ...
        for lang, data in sorted(stats.items()):
            lines.append(f"  {lang}: {data['files']} files, {data['definitions']} definitions ({data['classes']} classes, {data['functions']} functions)")
...
def find_pattern(pattern: str, file_type: Optional[str] = None) -> str:
    """
    ...
    try:
        ...
        # Analyze directory
        analyses = _analyzer.analyze_directory(directory, recursive=True, file_patterns=file_patterns)
        ...
        for file_path, analysis in analyses.items():
            ...
            if pattern_lower in ["classes", "class"]:
                matches = [(d.name, d.line, "class") for d in analysis.definitions if d.kind.value == "class"]
            elif pattern_lower in ["functions", "function", "methods", "method"]:
                matches = [(d.name, d.line, d.kind.value) for d in analysis.definitions 
