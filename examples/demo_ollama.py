...
import asyncio
from coda.providers import OllamaProvider, Message, Role
...
def demo_streaming():
    ...
    messages = [
        Message(role=Role.USER, content="Explain Python decorators in 3 sentences.")
    ]
...
async def demo_async():
    ...
    messages = [
        Message(role=Role.USER, content="What are the benefits of async programming?")
    ]
    ...
    async for chunk in provider.achat_stream(
        [Message(role=Role.USER, content="List 3 Python async best practices.")],
        model=model_id
    ):
        console.print(chunk.content, end="")
