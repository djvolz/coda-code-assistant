...
from coda.base.providers import MockProvider, Message, Role, ToolCall
...
class TestMCPTool(BaseTool):
    ...
    async def execute(self, arguments: dict) -> ToolResult:
        ...
        return ToolResult(
            success=True,
            result={"answer": result, "expression": f"{a} {op} {b} = {result}"}
        )
...
async def test_agent_error_handling_with_mcp_tools():
    ...
    from coda.services.tools import tool_registry
    from coda.services.errors import ToolExecutionError
