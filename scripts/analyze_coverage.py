...
class CoverageAnalyzer:
    ...
    def load_coverage(self) -> dict:
        ...
        if not self.coverage_file.exists():
            raise FileNotFoundError(
                f"Coverage file {self.coverage_file} not found. Run pytest with --cov first."
            )
    ...
    def get_module_coverage(self) -> list[tuple[str, float, int, int]]:
        ...
        for file_path, file_data in self.coverage_data.get("files", {}).items():
            if file_path.startswith("coda/"):
                ...
                modules.append((file_path, coverage_percent, total_statements, missing_lines))
    ...
    def identify_priority_modules(
        self, modules: list[tuple[str, float, int, int]]
        ...
        for module, coverage, statements, _missing in modules:
    ...
    def generate_report(self):
        ...
        if categories["critical"]:
            ...
            print("-" * 60)
            for module, coverage, statements, _missing in categories["critical"][:10]:
        ...
        if categories["excellent"]:
            ...
            print("-" * 60)
            for module, coverage, statements, _missing in categories["excellent"]:
                print(f"{coverage:5.1f}% | {statements:4d} stmts | {module}")
...
def main():
    ...
    parser.add_argument(
        "--min-coverage", type=float, default=80.0, help="Minimum acceptable coverage percentage"
    )
