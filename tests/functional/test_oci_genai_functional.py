...
""")
    ...
    def test_cli_one_shot_mode(self, temp_config):
        ...
        result = self.run_coda_command([
            "--one-shot", "Say 'xAI test OK' and nothing else",
        ...
        result = self.run_coda_command([
            "--one-shot", "Say 'Cohere test OK' and nothing else",
        ...
        result = self.run_coda_command([
            "--one-shot", "Say 'Meta test OK' and nothing else",
...
"""
    ...
    def test_concurrent_requests(self):
        ...
        def make_request(index):
            result = self.run_coda_command([
                "--one-shot", f"Say 'Request {index} OK'",
                "--model", "xai.grok-3-fast"
            ])
...
"""
    ...
    def test_special_characters_handling(self):
        ...
        for input_text in special_inputs:
            result = self.run_coda_command([
                "--one-shot", f"Echo this exactly: {input_text}",
                "--model", "xai.grok-3-fast"
            ])
