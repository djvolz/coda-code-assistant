...
from coda.agents import Agent, tool, FunctionTool
from coda.providers.base import ChatCompletion, Tool, ToolCall, Message, Role
...
class TestToolCallingFlow:
    ...
    def test_multiple_tool_calls_in_sequence(self, mock_provider, recorder):
        ...
        with patch.object(FunctionTool, 'execute', mock_execute):
            ...
            response = agent.run("Add 10 and 20, then multiply 5 and 6")
    ...
    def test_tool_call_error_handling(self, mock_provider, recorder):
        ...
        def mock_chat(messages, model, tools=None, **kwargs):
            ...
            if len(recorder.requests) == 1:
                ...
                if tool_msg and "Error" in tool_msg.content:
                    ...
                    response = ChatCompletion(
                        content="The result is calculated.",
                        model=model,
                        finish_reason="stop"
                    )
        ...
        with patch.object(FunctionTool, 'execute', mock_execute):
            ...
            response = agent.run("Divide 10 by 0")
        ...
        final_response = summary["responses"][1]
        assert "cannot divide" in final_response["content"].lower() or "error" in final_response["content"].lower()
...
class TestProviderToolFormat:
    ...
    def test_tool_call_id_handling(self):
        ...
        response = agent.run("Add 1 and 2")
