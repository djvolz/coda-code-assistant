...
class TestModelSelector:
    ...
    def mock_state_utils(self):
        ...
        with patch('coda.web.components.model_selector.get_state_value') as mock_get, \
    ...
    def sample_models(self):
        ...
        return [
            Model(id="gpt-4", name="GPT-4", provider="openai", context_length=8192, max_tokens=4096),
            Model(id="gpt-3.5-turbo", name="GPT-3.5 Turbo", provider="openai", context_length=4096, max_tokens=4096),
            Model(id="gpt-4-turbo", name="GPT-4 Turbo", provider="openai", context_length=128000, max_tokens=4096),
        ]

    def test_render_with_provider_selected(self, mock_streamlit, mock_state_utils, mock_provider_factory, sample_models):
    ...
    def test_model_persistence(self, mock_streamlit, mock_state_utils, mock_provider_factory):
        ...
        ollama_models = [
            Model(id="llama2", name="Llama 2", provider="ollama", context_length=4096, max_tokens=4096),
            Model(id="codellama", name="Code Llama", provider="ollama", context_length=4096, max_tokens=4096),
            Model(id="mistral", name="Mistral", provider="ollama", context_length=8192, max_tokens=4096),
        ]
    ...
    def test_provider_switch_model_update(self, mock_streamlit, mock_state_utils, mock_provider_factory):
        """Test model update when switching providers."""
        mock_get, mock_set = mock_state_utils

        litellm_models = [
            Model(id="claude-2", name="Claude 2", provider="litellm", context_length=100000, max_tokens=4096),
            Model(id="claude-instant", name="Claude Instant", provider="litellm", context_length=100000, max_tokens=4096),
        ]
    ...
    def test_provider_error(self, mock_streamlit, mock_state_utils, mock_provider_factory):
        ...
        # Should show error
        mock_streamlit.error.assert_called_once_with("Error loading models for OPENAI: Provider error")
        assert result is None

    def test_model_format_function(self, mock_streamlit, mock_state_utils, mock_provider_factory, sample_models):
