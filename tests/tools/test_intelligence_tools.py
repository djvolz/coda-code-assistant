...
''')
    ...
    def test_analyze_file_tool(self):
        ...
        result = asyncio.run(tool.execute({"file_path": str(self.python_file), "include_references": True}))
        assert result.success
    ...
    def test_find_definition_tool(self):
        ...
        result = asyncio.run(tool.execute({"name": "test_function", "directory": str(self.test_dir)}))
        assert result.success
        assert result.result["count"] == 1
        assert result.result["definitions"][0]["kind"] == "function"
        
        # Test finding with kind filter
        result = asyncio.run(tool.execute({"name": "method1", "directory": str(self.test_dir), "kind": "method"}))
        assert result.success
        assert result.result["count"] == 1
        ...
        # Test invalid kind
        result = asyncio.run(tool.execute({"name": "TestClass", "directory": str(self.test_dir), "kind": "invalid"}))
        assert not result.success
