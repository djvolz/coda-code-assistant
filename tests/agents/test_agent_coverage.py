...
from coda.providers.base import BaseProvider, Message, Role, Tool, ToolCall, ChatCompletion, Model
...
class TestAgentAdditional:
    ...
    async def test_run_async_with_tool_calls(self, agent):
        ...
        response = await agent.run_async(
            "Process this text",
            on_fulfilled_action=mock_callback
        )
    ...
    async def test_run_async_tool_call_loop_detection(self, agent):
        ...
        agent.provider.chat.side_effect = [
            ...
            ChatCompletion(content="", model="test.model", finish_reason="tool_calls", tool_calls=[tool_call]),
            ChatCompletion(content="", model="test.model", finish_reason="tool_calls", tool_calls=[tool_call]),
            ChatCompletion(content="", model="test.model", finish_reason="tool_calls", tool_calls=[tool_call]),
    ...
    def test_execute_tool_calls(self, agent):
        ...
        tool_calls = [
            ToolCall(id="1", name="sample_function", arguments='{"text": "hello"}')
        ]
    ...
    def test_execute_tool_calls_invalid_json(self, agent):
        ...
        tool_calls = [
            ToolCall(id="1", name="sample_function", arguments='invalid json')
        ]
