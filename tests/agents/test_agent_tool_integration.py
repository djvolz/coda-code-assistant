...
class TestAgentToolIntegration:
    ...
    async def test_single_tool_execution(self):
        ...
        assert response.performed_actions[0].function_name == "test_add"
        assert response.performed_actions[0].result == 8
    ...
    async def test_multiple_tool_calls(self):
        ...
        # Run agent
        response = await agent.run("Add 2 and 3, then multiply 5 by 4")
        
        # Check response
        assert "5" in response.content
        assert "20" in response.content
        assert len(response.performed_actions) == 2
        assert response.performed_actions[0].result == 5
        assert response.performed_actions[1].result == 20
    ...
    async def test_async_tool_execution(self):
        ...
        # Run agent
        response = await agent.run("Process the text 'hello'")
        
        # Check response
        assert response.performed_actions[0].result == "Processed: HELLO"
    ...
    async def test_tool_error_handling(self):
        ...
        # Run agent
        response = await agent.run("Run the error tool")
        
        # Check error was handled
        assert "error" in response.content.lower()
        assert len(response.performed_actions) == 1
        assert "Intentional test error" in response.performed_actions[0].error
    ...
    async def test_complex_data_handling(self):
        ...
        # Run agent
        response = await agent.run("Get complex data")
        
        # Check response
        assert response.performed_actions[0].result == {
            "status": "ok",
            "data": [1, 2, 3],
            "nested": {"key": "value"}
        }
    ...
    async def test_builtin_tools_integration(self):
        ...
        assert len(response.performed_actions) == 1
        assert response.performed_actions[0].function_name == "get_datetime"
        # Result should be ISO format datetime string
        assert "T" in response.performed_actions[0].result
    ...
    async def test_file_operations_integration(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Run agent
            response = await agent.run(f"Write 'Hello, World!' to {tmpdir}/test.txt then read it back")
            
            # Check operations
            assert len(response.performed_actions) == 2
            assert "Successfully wrote" in response.performed_actions[0].result
            assert response.performed_actions[1].result == "Hello, World!"
    ...
    async def test_tool_not_found(self):
        ...
        # Run agent
        response = await agent.run("Use unknown tool")
        
        # Check error handling
        assert len(response.performed_actions) == 1
        assert "not found" in response.performed_actions[0].error
    ...
    async def test_invalid_arguments(self):
        ...
        # Run agent
        response = await agent.run("Add invalid values")
        
        # Should handle the error gracefully
        assert len(response.performed_actions) == 1
        assert response.performed_actions[0].error is not None
    ...
    async def test_conversation_history_with_tools(self):
        ...
        response = await agent.run("Now add 3 and 4")
    ...
    async def test_parallel_tool_execution(self):
        ...
        # Run agent
        response = await agent.run("Do multiple calculations at once")
        
        # Check all operations completed
        assert len(response.performed_actions) == 3
        results = [action.result for action in response.performed_actions]
        assert 3 in results  # 1 + 2
        assert 12 in results  # 3 * 4
        assert "Processed: TEST" in results