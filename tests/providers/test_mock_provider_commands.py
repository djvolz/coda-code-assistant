...
class TestMockProviderCommands:
    ...
    def test_model_switching_scenario(self):
        ...
        assert "python" in response_smart.content.lower() and "decorator" in response_smart.content.lower()
    ...
    def test_session_command_scenarios(self):
        ...
        assert any(word in response_loaded.content.lower() for word in ["python", "flask", "django", "framework"])
    ...
    def test_mode_switching_workflow(self):
        ...
        for workflow in workflows:
            ...
            if workflow["final_check"] in messages[-1].content.lower():
                assert workflow["final_check"] in response.content.lower() or len(response.content) > 10