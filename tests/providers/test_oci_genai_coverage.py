...
from coda.providers.base import Message, Role, Tool, ToolCall, Model, ChatCompletion
...
class TestOCIGenAIProviderAdditional:
            ...
            with patch('builtins.open', create=True) as mock_open:
                with patch('coda.providers.oci_genai.GenerativeAiInferenceClient'):
                    with patch('coda.providers.oci_genai.GenerativeAiClient'):
                        ...
                        with patch.dict(os.environ, {}, clear=True):
                            with patch.object(OCIGenAIProvider, '_get_from_coda_config', return_value='config-compartment'):
                                provider = OCIGenAIProvider()
    ...
    def test_model_id_formatting(self, provider):
        ...
        provider._model_id_map["cohere.command-r-plus"] = "ocid1.generativeaimodel.oc1.us-chicago-1.xxx"
        provider._model_id_map["meta.llama-3.1-70b"] = "ocid1.generativeaimodel.oc1.us-chicago-1.yyy"
    ...
    async def test_achat_functionality(self, provider):
        ...
        provider.chat = Mock(return_value=ChatCompletion(
            content="Response",
            model="test.model",
            finish_reason="stop"
        ))
        ...
        with patch('coda.providers.oci_genai.GenerativeAiInferenceClient'):
            with patch('coda.providers.oci_genai.GenerativeAiClient'):
                with patch.dict(os.environ, {'OCI_COMPARTMENT_ID': 'test'}):
                    provider = OCIGenAIProvider(config_file_location="~/custom/config")
                    
                    # Check that expanduser was called
                    mock_oci_config.assert_called_with(
                        file_location=os.path.expanduser("~/custom/config"),
