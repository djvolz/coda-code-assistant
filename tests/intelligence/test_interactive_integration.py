...
class TestIntelligenceInteractiveCLI:
    ...
    def test_intelligence_commands_work_with_mock_provider(self):
        ...
        messages = [
            Message(role=Role.USER, content="Hello, I need help analyzing my Python code.")
        ]
        ...
        analyze_result = self.intel_commands.handle_command("analyze", [str(self.temp_dir / "main.py")])
    ...
    def test_intelligence_provides_context_for_ai_conversation(self):
        ...
        messages = [
            ...
            Message(
                role=Role.USER, 
                content="Can you help me understand the structure of my codebase?"
            )
        ]
    ...
    def test_dependency_analysis_for_ai_assistance(self):
        ...
        utils_analysis = self.intel_commands.handle_command("analyze", [str(self.temp_dir / "utils.py")])
        ...
        messages = [
            Message(
                role=Role.SYSTEM,
                content=f"Code analysis context: {context_info[:500]}..."
            ),
            Message(
                role=Role.USER,
    ...
    def test_graph_generation_for_visualization_context(self):
        ...
        export_result = self.intel_commands.handle_command("graph", [
            str(self.temp_dir),
            "--format=json",
            f"--output={graph_file}"
        ])
    ...
    def test_find_definitions_for_code_assistance(self):
        ...
        messages = [
            Message(
                role=Role.USER,
                content="Where is the DataProcessor class defined in my code?"
            )
        ]
    ...
    def test_error_handling_in_interactive_context(self):
        ...
        for command, args in error_cases:
            ...
            assert any(word in result.lower() for word in [
                "error", "not found", "usage", "unknown", "missing"
            ])
    ...
    def test_realistic_development_workflow(self):
        ...
        map_result = self.intel_commands.handle_command("map", [str(self.temp_dir)])  # Ensure mapped
        ...
        main_analysis = self.intel_commands.handle_command("analyze", [str(self.temp_dir / "main.py")])
