...
class TestInteractiveSession:
    ...
    def mock_oci_provider(self):
        ...
        provider = Mock()
        provider.list_models = Mock(return_value=[
            Mock(id='test.model.1', display_name='Test Model 1', 
                 metadata={'capabilities': ['CHAT']}),
            Mock(id='test.model.2', display_name='Test Model 2',
                 metadata={'capabilities': ['CHAT']}),
        ])
    ...
    @patch('coda.cli.interactive.OCIGenAIProvider')
    async def test_run_interactive_session_basic(self, mock_oci_class, mock_cli_class, 
                                                mock_oci_provider, mock_interactive_cli):
    ...
    @patch('coda.cli.interactive.InteractiveCLI')
    @patch('coda.cli.interactive.OCIGenAIProvider')
    @patch('coda.cli.interactive.ModelSelector')
    async def test_run_interactive_session_model_selection(self, mock_selector_class,
                                                         mock_oci_class, mock_cli_class,
                                                         mock_oci_provider, mock_interactive_cli):
    ...
    @patch('coda.cli.interactive.InteractiveCLI')
    @patch('coda.cli.interactive.OCIGenAIProvider')
    async def test_run_interactive_session_error_handling(self, mock_oci_class, mock_cli_class,
                                                        mock_console):
