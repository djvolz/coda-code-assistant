...
class TestOCIGenAIIntegration:
    ...
    @pytest.mark.parametrize("model_prefix,expected_response_format", [
        ("xai", "message"),
        ("cohere", "text"),
        ("meta", "message")
    ])
    ...
    def test_system_message_handling(self, provider):
        ...
        for model in models[:3]:  # Test first 3 models
            ...
            uppercase_ratio = sum(1 for c in response.content if c.isupper()) / len(response.content.replace(" ", ""))
